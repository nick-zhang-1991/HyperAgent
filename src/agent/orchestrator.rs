...
impl Orchestrator {
    ...
    pub async fn run(&mut self, prompt: &str) -> Result<RunResult> {
      ...
      Errors remaining: {} unique, {} new this round",
        ...
        // Collect changed files before auto-commit
        let changed_files: Vec<String> = approved.iter()
            .map(|c| c.file.to_string_lossy().to_string())
            .collect();

        // Auto-commit: if changes were applied and we're in a git repo
        let has_git = self.root.join(".git").exists() || self.root.join("../.git").exists();
        if has_git && !changed_files.is_empty() {
            let add_result = std::process::Command::new("git")
                    .args(["-C", &self.root.to_string_lossy(), "add", "-A"])
                    .output()
                    .ok();
                if add_result.map_or(false, |r| r.status.success()) {
                    // Generate descriptive commit message from the prompt
                    let commit_msg = format!(
                        "🤖 {}\n\nAuto-generated by HyperAgent",
                        prompt.trim().trim_end_matches('.').trim()
                    );
                    let commit_result = std::process::Command::new("git")
                        .args(["-C", &self.root.to_string_lossy(), "commit", "-m", &commit_msg])
                        .output()
                        .ok();
                    if let Some(out) = commit_result {
                        if out.status.success() {
                            let stdout = String::from_utf8_lossy(&out.stdout);
                            let summary = stdout.lines().find(|l| l.contains("changed") || l.contains("insertion") || l.contains("file"))
                                .unwrap_or("committed");
                            println!("   📦 Auto-committed: {}", summary.trim());
                        } else {
                            let stderr = String::from_utf8_lossy(&out.stderr);
                            if !stderr.contains("nothing to commit") && !stderr.contains("no changes") {
                                tracing::debug!("Auto-commit failed: {}", stderr.lines().next().unwrap_or("?"));
                            }
                        }
                    }
                }
